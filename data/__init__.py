...
def run_preprocessing(iid: bool = True):
    ...
    for split in ['train', 'val', 'test']:
        split_path: str = f"{config.PROCESSED_DATA_PATH}/{split}/{'iid' if iid else 'noniid'}"
    ...
    for building_id in range(1, config.NUMBER_CLIENTS + 1):
        ...
        df = preprocessing.compute_net_consumption(df)
        df = preprocessing.reorder_columns(df)
        df = preprocessing.replace_nan_values(df)

        if iid:
            df = preprocessing.normalize_energy_data(df)
            df = preprocessing.normalize_weather_data(df)
        ...
        for split_name, split_df in zip(['train', 'val', 'test'], [traing_df, val_df, test_df]):
            ...
            output_file_name: str = f'{split_name}/{"iid" if iid else "noniid"}/building_{building_id}'
